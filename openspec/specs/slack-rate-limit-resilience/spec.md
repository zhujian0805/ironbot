# slack-rate-limit-resilience

## Purpose

Document how the agent avoids hitting Slack rate limits after long idle periods by tracking idle time, gating reconnection probes, and exposing cooldown state.
...
- **THEN** the agent waits for the cooldown timer instead of immediately sending uth.test and logs the cooldown state

### Requirement: Rate limiter integration for Slack probes
Slack health-check probes (e.g., uth.test, pps.connections.open) SHALL be funneled through the existing RateLimiter/RetryManager, ensuring each request honors the configured wait time before retrying.
